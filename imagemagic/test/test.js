...
describe("IR - Image Resizer", function() {
  ...
  xit("should throw an exception if given a negative number", function() {
    ...
    it("getImageFormatFromFilename format should be jpg for Wallpaper800x600.jpg", function() {
        ...
        // expect(format).toEqual('jpg'); // jasmine test
        assert.equal(format, 'jpg');
    });

    xit("getImageFormatFromFilename format should be png for rainbow.png", function() {
    ...
    xit("getImageFormatFromFilename should throw error for file without extension e.g: rainbow", function() {
    ...
    xit("getImageFormatFromFilename should throw error for file incorrect extension e.g: document.doc", function() {
    ...
    xit("stripDimensionsFromSourceImageName Should strip dimensions from Wallpaper800x600.jpg", function() {
    ...
    xit("stripDimensionsFromSourceImageName Should strip dimensions from mypic3381_x_2468.jpg", function() {
        var originalImage = 'mypic3381_x_2468.jpg';
        var stripped = IR.stripDimensionsFromSourceImageName(originalImage, 'jpg');
        expect(stripped).toEqual('mypic.jpg');
    });

    xit("stripDimensionsFromSourceImageName Should NOT strip numbers from andre3000.jpg", function() {
        var originalImage = 'andre3000.jpg';
        var stripped = IR.stripDimensionsFromSourceImageName(originalImage, 'jpg');
        expect(stripped).toEqual(originalImage);
    });

    xit("stripDimensionsFromSourceImageName Should NOT strip numbers from 2000ad.jpg", function() {
        var originalImage = '2000ad.jpg';
        var stripped = IR.stripDimensionsFromSourceImageName(originalImage, 'jpg');
        expect(stripped).toEqual(originalImage);
    });

    xit("stripDimensionsFromSourceImageName Should NOT strip anything from kittens.jpg", function() {
